...
func (this *CommonController) login(member models.Member) {
	...
	user.Avatar = this.completeLink(utils.ShowImg(user.Avatar))
...
func (this *CommonController) UserReleaseBook() {
	...
	for _, item := range res {
		...
		book.Cover = this.completeLink(utils.ShowImg(book.Cover))
...
func (this *CommonController) Read() {
	...
	cols := []string{"book_id", "privately_owned", "member_id", "identify", "is_free_read"}
	if bookId > 0 {
...
func (this *CommonController) GetComments() {
	...
	if len(comments) > 0 {
		for idx, _ := range comments {
			comments[idx].Avatar = this.completeLink(utils.ShowImg(comments[idx].Avatar))
		}
...
// 查询书籍评论
func (this *CommonController) GetShowComments() {
	this.Response(http.StatusOK, messageSuccess, 1)
}
